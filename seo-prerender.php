...
$SITE_DOMAIN   = $_SERVER['HTTP_HOST'] ?? 'apimstec.com';
...
// ── Parse route (apimstec-react: English default at root, no /en in URL) ─────
$path = parse_url(isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '/', PHP_URL_PATH);
$path = '/' . ltrim($path, '/');

// Legacy bookmarked URLs: /en/... → same content as English at root
...
function buildMetaTags($routeType, $data, $locale, $origin, $path)
{
    ...
    if (!$data || !$hasPayload) {
        switch ($routeType) {
            ...
            case 'tools':
                ...
                $tags['description'] = 'Tools and resources.';
                break;
            case 'tool-landing':
                $tags['title'] = 'ApimsTec';
                $tags['description'] = '';
    ...
    switch ($routeType) {
        ...
        case 'tools':
            $tags['title'] = 'All Tools';
            $tags['description'] = 'Browse tools and resources.';
            $tags['og_title'] = $tags['title'];
            $tags['og_desc'] = $tags['description'];
            if (!empty($data['json_ld'])) {
                $tags['json_ld'] = $data['json_ld'];
